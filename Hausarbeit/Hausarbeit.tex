...
\begin{enumerate}
    ...
    \item die sukzessive Rang-k-Approximationen zu erzeugen und zu analysieren,
...
\]
darstellen. Jeder Eintrag $a_{ij}$ stellt dabei den Helligkeitswert eines Pixels dar. Binäre Bilder, wie das in dieser Hausarbeit benutzte „F", enthalten nur die Werte 0 und 1. Sie eignen sich trotzdem hervorragend zu Untersuchung von Strukturzerlegung, da sie klare Kanten und Zusammenhänge besitzen.
...
Die Analyse der Speicherersparnis zeigte, dass die SVD-basierte Speicherung nur dann vorteilhaft ist, wenn der Rang $k$ klein im Verhältnis zu den Matrixdimensionen $m$ und $n$ ist. Für zu große Werte von $k$ übersteigt der Speicherbedarf der SVD-Rekon-struktion sonst sogar den der Originalmatrix. Dies unterstreicht die Notwendigkeit einer sorgfältigen Wahl des Kompressionsgrades basierend auf den spezifischen Anforderungen an Speicherplatz und Rekonstruktionsqualität.
